...
def _audit(result: dict, head: str) -> str:
    ...
    lines = [
        ...
        "## Quality",
        "",
        f"- valid {quality.get('valid_n')} invalid {quality.get('invalid_n')} spy {quality.get('spy_status')}",
        f"- execution gates {quality.get('execution_gates')}",
        "",
        "## 864 plan",
        "",
        f"- plan_n {result.get('plan_n')} mid completed {mid.get('completed_n')} pending {progress.get('pending_n')}",
        f"- signature {progress.get('signature')}",
        "",
        "## Selection vs background",
        "",
        f"- selection {result['selection_sets'].get('status')} own-identical days {result['selection_sets'].get('own_sets_identical_days')}",
