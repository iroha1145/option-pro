...
root = Path(sys.argv[1]); out = root / "out5"; out.mkdir(exist_ok=True)
...
V = {}
vs126 = pct(R["126_21"] / vol126); vs252 = pct(R["252_21"] / (lr.rolling(252).std() * math.sqrt(252)))
M_vs_mid = 0.6 * vs126 + 0.4 * vs252
res12_sum = resid.rolling(231).sum().shift(21); res12_std = resid.rolling(231).std().shift(21)
res12_t = res12_sum / (res12_std * math.sqrt(231))
res6_t = resid.rolling(105).sum().shift(21) / (resid.rolling(105).std().shift(21) * math.sqrt(105))
V["V0_current_like"] = 0.50 * P["resid"] + 0.30 * T_prod + 0.20 * R_stab
V["D63skip5_resid_only"] = P["resid"]
V["D12m1_resid_only"] = pct(res12_t)
V["D6m1_resid_only"] = pct(res6_t)
V["D12m1_plus_Mvs"] = 0.5 * pct(res12_t) + 0.5 * M_vs_mid
V["M_vs_mid_only"] = M_vs_mid
V["D_like_res12_T30_R20"] = 0.50 * pct(res12_t) + 0.30 * T_prod + 0.20 * R_stab
V["D_like_res12_T30_Rneutral"] = 0.50 * pct(res12_t) + 0.30 * T_prod + 0.20 * 50.0
# gates
