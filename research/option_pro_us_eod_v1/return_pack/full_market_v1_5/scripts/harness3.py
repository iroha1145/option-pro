...
root = Path(sys.argv[1]); out = root / "out3"; out.mkdir(exist_ok=True)
...
V = {}
vs63 = pct(R[63] / vol63); vs126 = pct(R["126_21"] / vol126); vs252 = pct(R["252_21"] / (lr.rolling(252).std() * math.sqrt(252)))
V["V0_current_like"] = 0.50 * P["resid"] + 0.30 * T_prod + 0.20 * R_stab
V["V2_volscaled_mid"] = P["volscaled"]
V["V2s_volscaled_short"] = 0.5 * vs63 + 0.5 * vs126
V["V2l_volscaled_long"] = 0.5 * vs126 + 0.5 * vs252
V["V15a_M_volscaled_T_noR"] = 0.55 * (0.5 * vs63 + 0.5 * vs126) + 0.30 * T_prod + 0.15 * P["resid"]
V["V15b_M_volscaled_T_halfR"] = 0.50 * (0.5 * vs63 + 0.5 * vs126) + 0.25 * T_prod + 0.15 * P["resid"] + 0.10 * R_stab
V["V15c_M_volscaled_only_plus_resid"] = 0.7 * (0.5 * vs63 + 0.5 * vs126) + 0.3 * P["resid"]
V["V14_like_alpha05"] = 0.50 * (0.5 * P["resid"] + 0.5 * V["V2_volscaled_mid"]) + 0.30 * T_prod + 0.20 * R_stab
...
G = {
    ...
    "atr<=2.0xMed": atr_pct.le(2.0 * med_atr, axis=0),
    "atr<=3.0xMed": atr_pct.le(3.0 * med_atr, axis=0),
    "ext<=2": ext_atr <= 2.0,
    "ext<=3": ext_atr <= 3.0,
