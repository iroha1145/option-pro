...
root = Path(sys.argv[1]); out = root / "out2"; out.mkdir(exist_ok=True)
...
V["V1_raw_RS"] = 0.40 * P["r63"] + 0.30 * P["r126_21"] + 0.30 * P["r252_21"]
V["V2_volscaled_mom"] = P["volscaled"]
V["V3_52wH_plus_RS"] = 0.50 * P["dist52"] + 0.50 * V["V1_raw_RS"]
V["V4_RS_plus_accel"] = 0.60 * V["V1_raw_RS"] + 0.40 * P["accel"]
V["V5_clenow"] = P["clen"].where((adj > sma100) & (maxmove90 < 0.15))
V["V6_v14_like"] = 0.50 * (0.9 * P["resid"] + 0.1 * V["V1_raw_RS"]) + 0.30 * T_prod + 0.20 * R_stab
V["V7_RS_trend_noStab"] = 0.50 * V["V1_raw_RS"] + 0.30 * T_prod + 0.20 * P["dist52"]
V["V8_RS_resid_blend"] = 0.50 * V["V1_raw_RS"] + 0.50 * P["resid"]
V["V9_volscaled_52wH"] = 0.50 * P["volscaled"] + 0.50 * P["dist52"]
V["V10_volscaled_RS_accel"] = 0.40 * P["volscaled"] + 0.30 * V["V1_raw_RS"] + 0.30 * P["accel"]
V["Vx_R_stability_only"] = R_stab
V["Vx_D_resid_only"] = P["resid"]
V["Vx_T_trend_only"] = T_prod
V["V11_balanced_mix"] = 0.35 * P["volscaled"] + 0.25 * V["V1_raw_RS"] + 0.25 * T_prod + 0.15 * P["dist52"]
# gates
...
G = {
    ...
    "ext<=2&atr<=1.75xMed": (ext_atr <= 2.0) & atr_pct.le(1.75 * med_atr, axis=0),
    "atr<=2.5xMed": atr_pct.le(2.5 * med_atr, axis=0),
    "ext<=4&atr<=2.5xMed": (ext_atr <= 4.0) & atr_pct.le(2.5 * med_atr, axis=0),
}
# forward returns
