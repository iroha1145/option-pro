...
- `2510ef2e` **push** run 35151265134：**success**（21:14:41Z–21:43:20Z，约 28m39s）。pytest、前端、Playwright、空白检查、镜像、worker、视觉证据全部通过。
- `2510ef2e` **pull_request** run 35151267626：**success**（21:14:41Z–21:42:38Z，约 27m57s）。
- `879152db` **push** run 35154064843：**success**（21:44:55Z–22:13:12Z，约 28m17s）。pytest、前端、Playwright、镜像、worker、视觉证据全部通过。
- `879152db` **pull_request** run 35154068670：**success**（21:44:59Z–22:14:11Z，约 29m12s）。
- 此后若再推送报告层契约字段，新 head 在 GitHub job 完成前不声称端到端已验证。
...
| 5 不把缺失补成已知 | 已修 | NaN 保留；OHLC 违规丢弃；vintage/adjustment/halt/tradable 传到快照与信号摘要；缺量不当 0 量；本地 parquet 不默认 US/CS、不把 open 当成 raw；读入保留 halt/adjustment；停牌日账本拒买 |
...
| 7 完整 CI 含前端/镜像/worker | `879152db` 已绿 | push 35154064843 与 PR 35154068670 均 success（pytest / 前端 / Playwright / 镜像 / worker）；后续报告层字段 SHA 另记 |
