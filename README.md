...
未设置该变量、使用环境模板或运行交互式安装脚本时，均保持关闭。显式开启后，日线缓存会等待收盘数据结算 30 分钟，并且只有预期股票覆盖率达到 90% 才会写入全天缓存；覆盖不足或数据仍落后时只使用短时降级缓存。每日强度缓存保留 30 天；焦点快照最近 30 天保留全部半小时版本，30 至 90 天每个交易日保留代表版本，同时保留市场焦点周期和任务引用的完整输入。
