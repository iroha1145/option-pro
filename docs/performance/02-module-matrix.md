...
|------|------|--------------|----------|----------|--------|----------|
| 首页 | `/` | indices/status/regime/signals/strength/breakouts/earnings/watchlist/cta/calendar/data-status | 指数跳转、区块重试、日历切换、实时报价订阅 | 有（契约）视觉弱 | 间接 | 待测 |
| 自选 | `/watchlist` | market status、watchlist、signals、strength、account watchlist | 排序、增删、强制刷新、渐进列表、表格/卡片 | 有 | 有 | 待测 |
| 选股 | `/screener` | strength scan/market/profiles；扫描时 batch catalysts | 筛选、扫描、分页 20、行展开 | 有 | 有 | 待测 |
| 雷达 | `/breakouts` | status/current/events | 筛选、立即扫描、事件详情、历史分页 100 | 弱～有 | 有 | 待测 |
| 板块 | `/sectors` | sectors、strength/sectors、iv-ranking | 热力/列表、周期、IV | 弱 | 弱 | 待测 |
| 财报 | `/earnings` | upcoming；Owner refresh；AI impact | 周/月历、分页 24、分析任务 | 有 | 有 | 待测 |
| 新闻 | `/catalysts` | status、hotspots/status、feed×2、hotspots、focus latest/previous、Owner analysis-progress | 筛选/搜索/分类、游标分页 12、详情抽屉、刷新 | 有 | 有 | **优先** |
| 大盘 | `/market` | 同首页核心 + macro conditions/history | 指数聚焦、宏观刷新 | 有 | 有 | 待测 |
| CTA | `/cta` | market/cta、strength/market | instrument 切换 | 弱 | 间接 | 待测 |
| 个股 | `/stock/:ticker` | detail+technical 并行；预取 chart/signals；期权/新闻/绘图 | 周期、绘图、期权链、手动拉取 | 有 | 有 | 待测 |
| 登录 | `/login` | access/status；login/register | Owner/客户登录 | 弱～有 | 有 | 待测 |
| 404 | `*` | 无 | 返回首页 | 无 | 无 | 待测 |
