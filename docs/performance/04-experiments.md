...
- **C**：中文入口 gzip 约 79KB vs 基线 326KB；词典块只在 en/ja 下载。
- **D/E**：实现已合入工作分支；浏览器滚动与意图预取对照尚未写完。
- **复查修补**：`qs()` 不能把 `newsToday` / `tickerSummaries` 也默认成 visible。列表与 theme-boot 仍带 `page_mode=visible`；今日计数恢复为 24h/50 旧切片，摘要口径不变。生产包等 n=20 交错结束后再同步。
- **浏览器交错（9/20，未完成）**：优化冷 p50 9072 / p75 9309，0 超时；优化热 p50 2408 / p75 2477。未优化冷 8/9 超时（1 次 176s）；未优化热 7/9 超时。标题「芯片企业发布最新进展」。不得把这 9 对写成最终 n=20。
- **决定**：A/B/C 保留。D 正确性保留，等滚动样本。E 等对照，不明显则回退。newsToday 去 visible 保留。
