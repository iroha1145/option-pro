...
更新：2026-09-13 第 2 轮隔离复查已落盘；测量脚本已补齐，**结果尚未跑满**。Goal **未**标记完成。
...
- 桌面 / 360 / 430 关键场景补到 n=20；站内 SPA 导航；交错对照（脚本已写，等 soak 结束后跑）
- CTA / 个股 / 404 已纳入 `measure_pages.mjs`；n=20 未跑
- ≥2h 长稳进行中；RSS 采样已另进程记录；故障脚本已改成断言 stale-while-error，结果待 soak 后跑
- 用户点刷新绕过失败退避：源码已改，生产 `frontend/` 等 soak 结束后重建
- 独立三轮复查：第 1 轮对照清单 + 第 2 轮隔离复查；缺第 3 轮
- 真机、Safari、日本到美国链路：**待验证，未执行**

滚动 72h 窗随墙钟滑动：种子 `published_at` 锚定 2026-09-13T12:00Z，墙钟越往后，`summary.count` 会从基线 9566 下降（14:50Z 实测 9446）。这是产品时效，不是缩数据。对照与复测必须记当时 `count` / `as_of`，不能拿过期的 9566 当恒等式。
