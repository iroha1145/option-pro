...
`.env` 只保留一个迁移版本的兼容用途。进程已经导出的值优先级最高；`machine.env` 只提供七个机器字段，`secrets.env` 只提供八个密钥，错放字段不能覆盖正式来源。各业务模块接收同一个类型化配置对象，不再自行解释零散环境开关。
...
模型固定为 GPT-5.6 Terra，推理等级为 `max`。手动与定时任务各有一个提交槽，同类任务依次执行，最多同时运行一个手动任务和一个定时任务。新闻来源可以保留原文，但页面标题、摘要、等待提示和分析内容必须使用简体中文。

每日词元（Token）上限由 `daily_token_limit` 设置，默认 1000 万。提交前以已结算用量加下一笔任务预留判断额度，终态按已有用量规则结算；并发、幂等、冷却和未知提交保护继续有效。美元费用仍有本地记录供核对，但不作为任务准入限制，最终对账以供应商账单为准。
