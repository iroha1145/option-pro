...
Option Pro 常驻两个容器。`backend` 不运行长循环或付费任务；`worker` 持有唯一进程锁，在同一进程内隔离十四类任务。`sector_iv_refresh` 独立刷新全部板块的期权隐含波动率，并消费匿名或登录用户提交的有界刷新需求；它不依赖密码模式下的 `public_home`，也不受 `visitor_live_pulls` 开关控制。`strength_refresh` 每天在美东 22:00（收盘后的数据补齐缓冲期结束时）更新默认选股快照，部署或崩溃重启后沿用状态库里的下一次时刻，并消费参数化刷新请求；其他任务继续使用各自的启用条件和计划。某类任务失败时只更新自身状态并退避，不拖垮其他任务。
...
- 范围：`optix.db`、`catalyst-cache.db`、`ai-jobs.db`、`optix-worker.db`、`macro-conditions.db`、`accounts.db`（客户账户、自选列表和图表手绘线，唯一无法从数据源重建的库），以及 `runtime-settings.json`。文件与数据库使用同一套清单、SHA-256 摘要和保留规则。
- 频率：`maintenance` 任务每 6 小时一轮。每轮先备份全部文件；某个文件失败时，之后只重试这一个，间隔从 5 分钟起翻倍，最长 1 小时，其余文件等下一轮，不会被反复重备。
- 重启：下一次运行时刻在开始备份前就写进状态库，备份途中部署或崩溃，重启后沿用这个时刻，不会立刻从头再备份一遍；重启后的第一轮会跳过半个周期内已经有备份的文件。手动 `retention` 例外，每次都先把全部文件备份一遍再清理。
- 复制：SQLite 备份接口在一个读事务里一次复制完，预写日志（WAL）模式下不阻塞写入，也不会因为别的连接提交而从头重来；复制后做完整性检查（`integrity_check`）和外键检查，清单最后写入，作为提交标记。
- 保留：`[storage] backup_keep` 是每个文件最多保留的份数，也就是磁盘上限（约为文件大小乘以份数）。在这个份数内按时间分层挑选：最近 24 小时每轮一份，更早的每天只留最新一份，一周以前每周只留一份；超过份数时先删最旧的。默认 7 份约覆盖最近 3 天，11 份约一周，14 份约三周。
- 磁盘：复制前先把旧备份清到 `backup_keep - 1` 份，给新副本腾出位置，但至少留下最新的一份，直到新副本校验通过；连续失败时只腾这一次，不会因为时间推移继续删旧备份；随后检查剩余空间，不足「文件大小加 256 MiB 余量」时跳过这次复制，任务错误码为 `backup_insufficient_space`。
- 隔离：备份文件被手工删掉、只剩清单的记录会移到 `backups/quarantine/`，不再让这个文件的备份一直失败。
- 这些备份与正本在同一个数据卷上，只防误删和数据库损坏；发布前的备份仍按[迁移说明](migration.md)放到数据卷之外。
