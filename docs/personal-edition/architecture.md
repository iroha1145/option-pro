...
公开研究接口采用明确的路由清单，不按整段路径放行。访客只读取仍在允许时限内的进程缓存或已经保存的工作进程、轻量数据库（SQLite）和文件快照；没有可用快照时返回 503，不会补做外部请求、后台刷新、修改应用数据或保存刷新结果。公开新闻响应也不会初始化模型任务库、查询预算和令牌用量，或公开活动任务信息。
