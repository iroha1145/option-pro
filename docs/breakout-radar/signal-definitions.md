...
- 事件锚位固定为产生该事件的完整开盘区间高点；回踩、收复、延伸和距离评分
  都使用这一锚位，不借用日线整理区阻力。完整 5 分钟收盘跌破同一区间低点
  才构成该区间的失效证据，期间发生的破位不能被后来反弹抹去。
- 日线整理结构继续保留为背景。旧事件可从同一交易日已保存的完整区间与
  `pivot_id` 恢复锚位；仅有高点时失效位保持缺失，不能用日线低点或次日
  开盘区间补齐。已写入的历史轨迹不重造，后续状态按恢复后的原事件锚位观察。
...
一轮扫描可以在同一证据时刻连续发生多个状态变化。轨迹按证据时间排列，
同刻按数据库真实插入顺序 `transition_sequence` 排列，不能按随机事件编号
或状态等级排列；例如 CONFIRMED → RETESTING 是合法回退。
