...
`GET /api/integrations/macrolens/v1/focus-context` 只对精确 GET 方法豁免浏览器 `APP_AUTH_TOKEN`，随后仍须通过独立 HMAC、HTTPS、CIDR、防重放、TrustedHost 和全局限流。相同路径的 POST 不豁免。响应契约为 `contracts/option-pro-macrolens-focus-v2.json`，并携带固定 `schema_sha256`；任何正式强势、突破、市场或期权评分均不在响应中。
