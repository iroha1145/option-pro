...
test('数据源卡映射真实近24小时条数与新鲜度滞后', async () => {
  ...
  assert.equal(panel.includes('滞后时间表示数据的新旧程度'), true);
});

test('今日新闻优先使用完整过滤窗口汇总而非首屏五十条', async () => {
  const feedPath =
    '/catalysts/feed?window_hours=24'
    + '&include_unanalyzed=true&include_neutral=true&page_mode=visible&limit=50';
  const loaded = loadCatalystsModule({
    [feedPath]: {
      items: [
...
test('状态栏不再把产品名伪装成文章来源', () => {
  ...
  assert.equal(source.includes('新闻保留原始来源'), true);
});
