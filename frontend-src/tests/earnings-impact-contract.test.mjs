...
test('财报页面保留近期已公布结果并默认收纳长列表', () => {
  ...
  assert.equal(rightColumn.includes('<EpsHatchChart'), false);
  assert.equal(rightColumn.includes('<DensityStrip'), true);
  assert.equal(page.includes('useNow(1000)'), false);
  assert.equal(page.includes('useEtCalendarDate'), true);
  assert.equal(page.includes('<EarningsRefreshButton'), true);
  const deferred = fs.readFileSync(path.join(earningsComponentsPath, 'DeferredEpsChart.tsx'), 'utf8');
  assert.equal(deferred.includes('IntersectionObserver'), true);
