...
export const SECTORS: Dict = {
  ...
  '板块平均收益热力矩阵': ['Sector average return heat matrix', 'セクター平均リターン・ヒートマップ'],
  ...
  '操作': ['Actions', '操作'],
  '板块排位是同板块成分之间的横向比较，不是该股自己的历史高低位；期权与价格均为延迟数据': ["Sector rank is a cross-sectional comparison among that sector's constituents, not the stock's own historical high or low; options and prices are both delayed data.", 'セクター順位は同じセクター内の構成銘柄同士を横断的に比較したものであり、その銘柄自身の過去の高値・安値ではありません。オプションと価格はいずれも遅延データです。'],
  ...
  '板块目录暂时为空，重试可重新拉取。': ['The sector directory is currently empty. Retry to fetch it again.', 'セクターリストは現在空です。再試行すると再取得できます。'],
