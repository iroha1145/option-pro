...
export const SECTORS: Dict = {
  ...
  '板块隐含波动率（IV）排名': ['Sector implied volatility (IV) ranking', 'セクター内の予想変動率（IV）ランキング'],
  '更新于': ['Updated', '更新'],
  '降序': ['Descending', '降順'],
  '升序': ['Ascending', '昇順'],
  '排位': ['Rank', '順位'],
  ...
  '当前隐含波动率较高的股票': ['Stocks with higher current implied volatility', '現在の予想変動率が高い銘柄'],
  ...
  '当前隐含波动率排名': ['Current implied volatility ranking', '現在の予想変動率ランキング'],
  ...
  '板块信息不完整，暂无法查询隐含波动率排名。': ['Sector information is incomplete, so implied volatility rankings cannot be loaded.', 'セクター情報が不足しているため、予想変動率のランキングを取得できません。'],
  '板块 IV 数据': ['Sector IV data', 'セクター IV データ'],
  'IV 数据暂不可用': ['IV data temporarily unavailable', 'IV データは一時的に利用できません'],
  '板块信息暂不可用，恢复后将自动更新。': ['Sector information is unavailable. Updates will resume automatically when it recovers.', 'セクター情報を取得できません。復旧すると自動で更新します。'],
  ...
  '隐含波动率数据加载失败，请重试。': ['Implied volatility data could not be loaded. Please retry.', '予想変動率のデータを読み込めませんでした。再度お試しください。'],
  '读取失败': ['Load failed', '読み込み失敗'],
  '板块强度更新失败，当前显示上次结果（已过期{time}）。': ['Sector strength failed to update. Showing previous results (stale by {time}).', 'セクターの強度を更新できません。前回の結果を表示しています（{time}前に更新期限を経過）。'],
};
