...
export const SMART_DRAWINGS: Dict = {
  "原{label} · 突破已确认": ["Former {label} · breakout confirmed", "元の{label} · 上抜け確認"],
  "原{label} · 跌破已确认": ["Former {label} · breakdown confirmed", "元の{label} · 下抜け確認"],
  "{label} · 历史结构": ["{label} · historical structure", "{label} · 過去の構造"],
  "{label} · 突破待确认": ["{label} · breakout pending", "{label} · 上抜け確認待ち"],
  "{label} · 跌破待确认": ["{label} · breakdown pending", "{label} · 下抜け確認待ち"],
  "{label} · 参考": ["{label} · reference", "{label} · 参考"],
  "价格缺口（日线）": ["Price gaps (daily)", "価格ギャップ（日足）"],
  "价格缺口 · 已回补": ["Price gap · filled", "価格ギャップ · 窓埋め済み"],
  "价格缺口 · 未回补": ["Price gap · unfilled", "価格ギャップ · 未埋め"],
  "深色为主要边界，细线为参考；虚线为延伸，淡色点线为历史结构": ["Dark: main boundary; thin: reference; dashed: extension; faint dotted: historical structure", "濃色：主要境界、細線：参考、破線：延長、薄い点線：過去の構造"],
