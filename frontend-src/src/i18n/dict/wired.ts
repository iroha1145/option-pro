...
export const WIRED: Dict = {
  ...
  '宏观适配 {fit}（{tone}）。加入宏观因素后的参考优先级：原优先级 {sign} {delta}，最多调整 4 分；当前排序保持不变。': ['Macro fit {fit} ({tone}). Reference priority with macro factors: original priority {sign} {delta}, adjusted by at most 4 points. The current ranking is unchanged.', 'マクロ適合度 {fit}（{tone}）。マクロ要因を加えた参考優先度は、元の優先度 {sign} {delta}です。調整幅は最大4点で、現在の表示順は変わりません。'],
  ...
  '设置版本 {version}': ['Settings version {version}', '設定バージョン {version}'],
  '{n} 秒': ['{n}s', '{n}秒'],
  '{n} 分': ['{n}m', '{n}分'],
  '{n} 小时': ['{n}h', '{n}時間'],
  ...
  "范围内 {eligible} · 新安排 {queued} · 已安排 {existing} · 数据无效 {invalid}": ["Eligible {eligible} · newly scheduled {queued} · already scheduled {existing} · invalid data {invalid}", "対象 {eligible} · 新規予定 {queued} · 予定済み {existing} · 無効なデータ {invalid}"],
  ...
  "暂时无法为 {name} 生成分析，请稍后重试。": ["Unable to start the analysis for {name}. Please try again later.", "{name} の分析を開始できません。しばらくしてから再試行してください。"],
  ...
  '价格、板块、分档与最低分筛选仅适用于已载入的 {returned} 只股票；已评分股票共 {screened} 只。': ['Price, sector, tier, and minimum-score filters apply only to the {returned} loaded stocks; {screened} stocks have been scored in total.', '価格・セクター・ランク・最低スコアの条件は、読み込み済みの{returned}銘柄に適用されます。採点済みの銘柄は合計{screened}件です。'],
  ...
  '市场宽度偏弱': ['Market breadth is weak', '市場の広がりが弱い'],
  '波动或信用压力偏高': ['Volatility or credit stress is elevated', 'ボラティリティまたは信用ストレスが高い'],
  '风险偏好价差偏弱': ['The risk-on spread is weak', 'リスクオン・スプレッドが弱い'],
