...
export const SCORE_HINTS = {
  ...
  breakoutIntrinsic: {
    title: '个股强度（0–100）',
    body: '来自选股页同一套 strength-v2 个股自身强度分（六族加权），只用该股价格/量能与对 SPY 的相对表现。',
  },
  ...
  avgStrength: {
    title: '平均强度（0–100）',
    body:
      '组内成分股 strength-v2 综合强度分的简单平均（无分的标的不计入）。用于横向比较组与组，个股仍以各自分数为准。',
  },
  ...
  sectorIvRank: {
    ...
    body:
      '该股平值期权隐含波动率（ATM IV，取自 Yahoo 期权链）在本板块成分股中的百分位：100 = 板块内 IV 最高。它比较的是同板块内的相对贵贱，不是历史分位。',
  },
  sectorAtmIv: {
    title: 'ATM IV（%）',
    body:
      '最接近现价的期权（平值）的隐含波动率年化值，来自 Yahoo 期权链。数值越高，期权市场定价的预期波动越大。',
  },
