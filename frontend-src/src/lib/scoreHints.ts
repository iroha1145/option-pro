...
const MISSING_AWARE = '数据缺失的分项不会按中性 50 计入，而是移出权重重新归一，并压低置信度。';
...
export const SCORE_HINTS = {
  ...
  marketRegime: {
    ...
    note: '该分同时驱动选股权重的动态调节（如弱市自动降低突破类信号权重）。',
  },
  regimeTrend: {
    title: '指数趋势（0–100）',
    body:
  ...
  breakoutQualityComposite: {
    title: '突破质量 · 综合（0–100）',
    body: '基底质量 45% + 确认强度 45% + 流动性质量 10% 的加权合成。',
  },
  breakoutLiquidity: {
    title: '流动性质量（0–100）',
    body:
  ...
  focusCycleAssessment: {
    title: '偏向与置信（AI 判断）',
    body:
      '偏向是模型对这只股票的方向 × 强度估计，界面按 ±5 显示：正=利多、负=利空，'
      + '绝对值代表预期影响幅度。置信是模型对自己这次判断的把握程度（0–100），'
      + '置信低通常意味着新闻信息量不足或含糊，应降低参考权重。',
    note: '两者都是模型判断，不是量价计算：偏向不是收益预测，置信不是胜率。',
  },
  netImpact: {
    title: '净影响（−5 ～ +5 · 非收益）',
    body:
  ...
  readingDataQuality: {
    title: '数据质量（0–100%）',
    body: '本次解读中有真实读数的指标占比。缺数据的指标不参与聚合也不冒充中性，占比低时应降低整套解读的参考权重。',
  },
  trendBiasFactors: {
    title: '四因子子分（0–100）',
    body:
...
export const STOCK_SIGNAL_HINTS: Record<string, ScoreHint> = {
  sma20_dist: {
    title: '距 20 日均线',
    body: '现价相对 20 日均线的偏离（%）。大幅高于→顶部证据（×8），大幅低于→底部证据。',
  },
  sma50_dist: {
    title: '距 50 日均线',
    body: '现价相对 50 日均线的偏离（%）。映射系数 ×5，方向同上。',
  },
  sma200_dist: {
    title: '距 200 日均线',
    body: '现价相对 200 日均线的偏离（%）。映射系数 ×2.5，衡量长期趋势的过热/超卖。',
  },
  rsi14: {
    title: 'RSI(14)',
    body: '高于 50 每一点计 3 分顶部证据，低于 50 每一点计 3 分底部证据；80 附近顶部证据即打满。',
  },
  return_20d: {
    title: '20 日涨跌',
    body: '近 20 个交易日涨跌幅（%）。上涨×3.5 计入顶部证据，下跌×3.5 计入底部证据。',
  },
  atr_percentile: {
    title: 'ATR 波动分位',
    body: '当前波动率在一年中的分位。高分位（>60/70）时顶底证据同时上升——高波动常出现在拐点附近。',
  },
  volume_zscore: {
    title: '成交量 Z 分数',
    body: '当日量相对 20 日均量的标准化偏离。放量同时增加顶部（×20）与底部（×10）证据：极端量能常伴随顶或底。',
  },
  obv_divergence: {
    title: 'OBV 背离',
    body: '20 日内 OBV 与价格的背离度。量在价先弱（正值）计顶部证据，量先强于价计底部证据。',
  },
  relative_strength_spy: {
    title: '相对 SPY 强度',
    body: '对 SPY 的相对涨跌（%）。持续跑赢×6 计顶部证据（拥挤/过热），持续跑输计底部证据。',
  },
  close_position: {
    title: '收盘位置',
    body: '收盘价在当日高低区间中的位置（0–100）。收在区间低位（<35）计顶部/转弱证据，收在区间高位（>65）计底部反转确认证据。',
  },
  macd_hist: {
    title: 'MACD 柱',
    body: 'MACD 柱状值。转负（动能向下）×150 计顶部证据，转正计底部证据。',
  },
};
...
export const MACRO_FACTOR_HINTS: Record<string, ScoreHint> = {
  fed_net_liquidity: {
    title: '联储净流动性（0–100 分）',
    body:
      '联储总资产减去财政部一般账户与隔夜逆回购余额，单位十亿美元。数值越高表示可用于金融体系的储备越多。',
    note: '方向：原值越高分数越高（在过去 5 年分位中的位置）。评分方式：5 年滚动历史分位。',
  },
  bank_reserves: {
    title: '银行准备金（0–100 分）',
    body:
...
export const MARKET_SIGNAL_HINTS: Record<string, ScoreHint> = {
  sma20_distance: { title: 'SPY 距 20 日线', body: '大盘对 20 日均线的偏离（%）。高于→顶部证据（×8），低于→底部证据。' },
  sma50_distance: { title: 'SPY 距 50 日线', body: '偏离映射 ×5，衡量中期过热/超卖。' },
  sma200_distance: { title: 'SPY 距 200 日线', body: '偏离映射 ×2.5，衡量长期趋势位置。' },
  rsi14: { title: 'SPY RSI(14)', body: '高于 50 计顶部证据、低于 50 计底部证据，每点 ×3。' },
  return_20d: { title: 'SPY 20 日涨跌', body: '上涨×4 计顶部证据，下跌×4 计底部证据。' },
  rsp_spy_5d: { title: 'RSP−SPY 5 日', body: '等权落后于市值权重（负值）→龙头集中上涨，计顶部证据 ×15；等权领先计底部修复证据。' },
  iwm_spy_5d: { title: 'IWM−SPY 5 日', body: '小盘落后（负值）计顶部证据 ×15，小盘领先计底部证据——口径同 RSP。' },
  qqq_spy_5d: { title: 'QQQ−SPY 5 日', body: '成长强于大盘（正值）计顶部拥挤证据 ×8，反之计底部证据。' },
  sectors_above_50dma: { title: '板块宽度', body: '站上 50 日线的板块比例。低于 45% 计顶部风险，高于 85% 计过热；低于 55% 同时累积底部证据。' },
  vix: { title: 'VIX', body: '低于 15 每点 ×5 计顶部自满证据；高于 20 每点 ×4 计底部恐慌证据。' },
  vix_percentile: { title: 'VIX 一年分位', body: '分位越低顶部证据越强（×0.8），分位越高底部证据越强。' },
  vix_5d_change: { title: 'VIX 5 日变化', body: '上行计顶部证据 ×3，下行计底部证据 ×3。' },
  credit_risk: { title: '信用价差（HYG/TLT）', body: '信用债走弱（负值）计顶部风险证据 ×8；走强计底部修复证据 ×5。' },
  yield_10y: { title: '10 年期利率', body: '高于 4% 每点 ×25 计顶部压力；显著低于 4% 计底部证据。' },
  yield_10y_20d_change: { title: '利率 20 日变化', body: '利率上行 ×80 计顶部压力证据，下行计底部证据。' },
};
