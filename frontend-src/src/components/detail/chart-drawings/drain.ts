...
export type PersistApi = {
  ...
  remove: (drawingId: string, expected: number) => Promise<unknown>;
  clearScope: (
    ticker: string,
    range: ChartRange,
    expected: number,
...
export async function drainPersistJob(args: {
  ...
  try {
    ...
    if (job.type === 'create' && job.drawing) {
      ...
      await api.remove(job.drawingId, expected);
      scopeRevision = expected + 1;
      apply = { action: 'deleteRevision', id: job.drawingId };
    } else if (job.type === 'clear') {
      await api.clearScope(job.scope.ticker, job.scope.range, expected, job.scope.adjustment);
      scopeRevision = expected + 1;
      ...
      return { foreign: false, kind: 'drop', apply: { action: 'none' }, conflict: null, ...settled };
    }
    ...
    if (!applies) {
      ...
      return { foreign: false, kind: 'superseded', apply: { action: 'none' }, conflict: null, lastServer, scopeRevision, ...settled };
    }
    const settled = settleJob({ outbox, job, kind: 'success' });
    return { foreign: false, kind: 'success', apply, conflict: null, lastServer, scopeRevision, ...settled };
  } catch (error) {
    if (!jobBelongsToScope(job, outbox.getScope())) return FOREIGN;
    ...
    if (job.type === 'update' && (code === 'drawing_not_found' || status === 404)) {
      ...
      try {
        ...
        outbox.replacePending(regeneratePersistOps(args.drawings, remote.drawings));
        ...
        if (!jobBelongsToScope(job, outbox.getScope())) return FOREIGN;
        const settled = settleJob({ outbox, job, kind: 'conflict' });
        return { foreign: false, kind: 'conflict', apply: { action: 'none' }, conflict: null, ...settled };
      }
    }
    ...
    if (!applies) {
      releaseInflight(outbox, job, 'drop');
      const settled = settleJob({ outbox, job, kind: 'superseded' });
      return { foreign: false, kind: 'superseded', apply: { action: 'none' }, conflict: null, ...settled };
    }
    if (failure === 'drop') {
      releaseInflight(outbox, job, 'drop');
      const settled = settleJob({ outbox, job, kind: 'drop' });
    ...
    return { foreign: false, kind: 'retry', apply: { action: 'none' }, conflict: null, ...settled };
  }
}
